...
The `packages/web` package uses **Vite** (not `Bun.serve()`). Use `@vitejs/plugin-react` and `@tailwindcss/vite`.
...
Env vars exposed to the frontend must be prefixed with `VITE_` (e.g. `VITE_RELAY_URL`). Access them via `import.meta.env.VITE_*`.
